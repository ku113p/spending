...
async def _get_client() -> AsyncGenerator[AsyncMongoClient, None]:
    async with AsyncMongoClient(Config.MONGO_URI) as client:
        yield client
...
async def _get_collection() -> AsyncGenerator[AsyncCollection, None]:
    async with _get_db() as db:
        yield db[Config.MONGO_COLLECTION_NAME]
...
async def init_db():
    async with _get_db() as db:
        ...
        if Config.MONGO_COLLECTION_NAME not in collections:
            await db.create_collection(Config.MONGO_COLLECTION_NAME)
            logger.info(f"Collection '{Config.MONGO_COLLECTION_NAME}' created.")
        else:
            logger.info(f"Collection '{Config.MONGO_COLLECTION_NAME}' already exists.")
